...
import { setTransitVehiclesVisible } from './gtfsCommon.js';
...
export async function applyPageLayers(page) {
    ...
    if (page.id === 'visual-quality') {
        ...
        // Tram/bus dots are off here: at eye level they read as HUD markers and
        // compete with the detection frames. The Display & transit toggle still
        // turns them back on for anyone who wants them.
        setTransitVehiclesVisible(false);
        const gtfsSwitch = document.getElementById('gtfsVehiclesSwitch');
        if (gtfsSwitch) gtfsSwitch.checked = false;
    } else {
        disposeVisualQualityIndex();
    }
